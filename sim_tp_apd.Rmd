...
For the normal distribution, the groups were sampled from populations with means: $A_1B_1$ = 0, $A_1B_2$ = 0.1, $A_2B_1$ = 0.2, $A_2B_2$ = 0.9. 

Lognormal: $A_1B_1$ = 0, $A_1B_2$ = 0, $A_2B_1$ = 0, $A_2B_2$ = 0.7. 

Contaminated normal: $A_1B_1$ = 0, $A_1B_2$ = 0.1, $A_2B_1$ = 0.2, $A_2B_2$ = 0.9. 

Contaminated lognormal: $A_1B_1$ = 0, $A_1B_2$ = 0, $A_2B_1$ = 0, $A_2B_2$ = 0.7. 

Poisson: $A_1B_1$ = 7, $A_1B_2$ = 7, $A_2B_1$ = 7, $A_2B_2$ = 9. 

Beta-binomial: all groups had 10 bins, s = 9, and r was set to $A_1B_1$ = 5.5, $A_1B_2$ = 5.5, $A_2B_1$ = 5.5, $A_2B_2$ = 9.
...
Under normality ANOVA on means performs best, followed by ANOVA on trimmed means and finally apd quantile method. HD and QT7 methods give very similar results.  
...
Lognormal distributions: power is low for the ANOVAs relative to the bootstrap methods, and much more so when making inferences on means vs. trimmed means. Quantile method most powerful for n>40.
...
Again, very poor performance of the ANOVA on means, but now ANOVA on trimmed means dominates the bootstrap methods.
...
ANOVA on means dominates. Similarly to what we saw in `sim_tp.Rmd`, the ANOVA on trimmed means performs extremely poorly because the *g-and-h* distributions have median zero. On the other hand, the mean's lack of robustness to skewness works to its advantage.
...
ANOVA on means dominates, followed by ANOVA on trimmed means. The bootstrap method using HD is close to ANOVA on trimmed means. The bootstrap method using QT7 has much lower power. 
...
ANOVA on means dominates again. Now bootstrap + HD gives very similar results to ANOVA on trimmed means. Again QT7 has low power.
...
No method dominates. In some situations ANOVA on means outperforms the others, yet in other situations, power completely collapses for the ANOVA on means but not for the quantile method. HD and QT7 perform similarly for continuous distributions, but in the presence of tied values, HD dominates QT7.  
...
legend <- get_legend(p_fdr_norm + guides(color = guide_legend(nrow = 2)) + theme(legend.position = "bottom", legend.box.margin = margin(0, 0, 0, 0)))
